...
- Status: active; P0 and P1.1 complete; P1.2.1 implemented with initial disposable rehearsal complete and PR #23 live-review remediation present in the candidate
- Priority: high
...
- Notes: The expanded dependency model, decision anchors, exclusions, phase acceptance gates, and cross-phase definition of done are canonical in `C:\Projects\Cubby\hermes-control\contexts\implementation-roadmap.md`. P0 and P1.1 are complete in the application branch history. P1.2.1 and its initial disposable rehearsal are complete; the PR #23 remediation candidate additionally serializes registration-policy mutation with owner recovery, preserves byte-exact high-friction acknowledgements, sanitizes unexpected CLI failures, and uses observed PostgreSQL lock waiters in its acceptance source. Fresh disposable rehearsal, publication, exact-tree re-review, deployment, and cutover remain separate approval gates; each later slice still requires exact-tree planning and the normal worktree/action approvals.
