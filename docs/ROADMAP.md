...
- Notes: Implementation and isolated verification are complete. Preflight service discovery supports both legacy JSON-array output and Docker Compose 5.2 newline-delimited JSON while preserving fail-closed validation. The slice remains bounded to the existing Docker Compose home-server deployment and its operator workflow; generalized hosting, remote monitoring, remote backup providers, PWA/offline behavior, and multi-household infrastructure remain out of scope. Deployment to the household stack remains separately approval-gated.
