...
- Status: active; P0 complete and P1.1 candidate awaiting exact-tree verification/review
- Priority: high
...
- Notes: The expanded dependency model, decision anchors, exclusions, phase acceptance gates, and cross-phase definition of done are canonical in `C:\Projects\Cubby\hermes-control\contexts\implementation-roadmap.md`. P0 is complete. The P1.1 candidate replaces household-derived deployment registration policy with separately protected platform authority, explicit audited bootstrap verification/binding/recovery, fail-closed platform settings, and additive migration compatibility. Runtime migration, real owner operations, deployment, and cutover remain separate approval gates; each later slice still requires exact-tree planning and the normal worktree/action approvals.
