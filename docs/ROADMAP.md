...
- Status: active; P0 and P1.1 complete; P1.2.1 candidate awaiting exact-tree verification/review
- Priority: high
...
- Notes: The expanded dependency model, decision anchors, exclusions, phase acceptance gates, and cross-phase definition of done are canonical in `C:\Projects\Cubby\hermes-control\contexts\implementation-roadmap.md`. P0 and P1.1 are complete in the application branch history. The P1.2.1 candidate makes ordinary server-local backup status/download fail closed on household-owned records and adds supported empty credential-backed target provisioning plus separate, audited host-local one-file/one-fresh-target recovery authorization without bearer tokens or implicit sole-household discovery. Runtime migration, real owner or backup-recovery operations, disposable rehearsal, deployment, and cutover remain separate approval gates; each later slice still requires exact-tree planning and the normal worktree/action approvals.
