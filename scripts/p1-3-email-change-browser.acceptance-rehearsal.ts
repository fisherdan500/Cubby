  ...
  const inspect = async (width: number, height: number, desktop: boolean) => {
    ...
    const result = await client.call("Runtime.evaluate", {
      expression: `(async()=>{
        ...
        setValue(named('Current password for password change'),${JSON.stringify(syntheticSecrets.activationCurrentPassword)});setValue(named('New password'),${JSON.stringify(syntheticSecrets.activationNextPassword)});await click('Change password');await waitFor(()=>document.querySelector('[aria-live="polite"]')?.textContent?.includes('Account security request could not be completed'),'password_loss_missing');if(document.body.textContent.includes('Synthetic response lost'))throw new Error('password_backend_detail_disclosed');
