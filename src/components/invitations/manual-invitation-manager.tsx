...
export function ManualInvitationManager({ invites, canInviteAdmin, isOwner }: { invites: Invite[]; canInviteAdmin: boolean; isOwner: boolean }) {
  ...
  return <section aria-labelledby="manual-invites-heading" className="space-y-4"><div><h2 id="manual-invites-heading" className="text-lg font-bold">Invite member</h2><p className="text-sm text-muted-foreground">Create a household invitation. The link is displayed once, only after completion.</p></div>
    <form onSubmit={(event) => void create(event)} className="space-y-3"><label className="block text-sm font-semibold">Recipient email<Input name="recipientEmail" type="email" autoComplete="email" required /></label><label className="block text-sm font-semibold">Access level<select name="role" defaultValue="caretaker" className="mt-1 min-h-11 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm"><option value="caretaker">Caretaker</option><option value="parent">Parent</option><option value="read_only">Read only</option>{canInviteAdmin ? <option value="admin">Admin</option> : null}</select></label><label className="block text-sm font-semibold">Expires in<select name="expiresInHours" defaultValue="168" className="mt-1 min-h-11 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm"><option value="1">1 hour</option><option value="24">1 day</option><option value="168">7 days</option></select></label><Button type="submit" disabled={busy}>{busy ? "Working…" : "Create invitation"}</Button></form>
