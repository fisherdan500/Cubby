...
describe("dashboard running timer indicators", () => {
  it("carries no timer buttons at all on the tile", () => {
    const body = renderMarkup(createElement(RunningTimerTile, { timer: timer(), label: "Sleep", nowMs, returnTo: "/app" }));
  ...
  it("opens the activity, with a way back to the dashboard", () => {
    const body = renderMarkup(createElement(RunningTimerTile, {
      timer: timer(),
      label: "Sleep",
      nowMs,
      returnTo: "/app?babyId=baby-1&date=2026-09-20&summaryType=sleep"
    }));

    expect(body.querySelector("a")?.getAttribute("href")).toBe(
      "/app/activities/timer-1?returnTo=%2Fapp%3FbabyId%3Dbaby-1%26date%3D2026-09-20%26summaryType%3Dsleep"
    );
  });
  ...
  it("shows a paused timer frozen at the moment it was paused", () => {
    ...
    const body = renderMarkup(createElement(RunningTimerTile, { timer: paused, label: "Sleep", nowMs, returnTo: "/app" }));
  ...
  it("gives a row the same indicator and no controls", () => {
    const body = renderMarkup(createElement(RunningTimerRow, { timer: timer({ type: "pumping" }), nowMs, returnTo: "/app" }));
  ...
  it("accepts an already-serialized instant as well as a Date", () => {
    const body = renderMarkup(
      createElement(RunningTimerRow, { timer: timer({ startedAt: "2026-09-21T10:00:00.000Z" }), nowMs, returnTo: "/app" })
    );
