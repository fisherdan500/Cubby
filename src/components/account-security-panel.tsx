...
async function post(path: string, payload: Record<string, unknown>) {
  ...
  if (!response.ok || body?.ok !== true || !body.data) throw new RequestError("Account security request could not be completed. Check status before retrying.", response.status, body?.error?.code);
  return body.data;
}
...
export function AccountSecurityPanel({ accountScope, onSignInRequired }: { accountScope: string; onSignInRequired?: () => void }) {
  ...
  const displayOnceRecoveryCodesRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => { if (recoveryCodes.length) displayOnceRecoveryCodesRef.current?.focus(); }, [recoveryCodes]);
  ...
  return <section className="space-y-6" aria-label="Account security controls" aria-busy={busy ? "true" : "false"}>
    <section className="space-y-3 rounded-lg border border-border bg-card p-4" aria-labelledby="change-password-heading"><h2 id="change-password-heading" className="font-editorial text-xl font-bold">Change password</h2><form aria-labelledby="change-password-heading" className="space-y-3" onSubmit={(event) => { event.preventDefault(); void changePassword(); }}><label htmlFor="password-current" className="text-sm font-semibold">Current password</label><input id="password-current" aria-label="Current password for password change" type="password" autoComplete="current-password" value={password.current} onChange={(event) => setPassword({ ...password, current: event.target.value })} className="min-h-11 w-full rounded-lg border border-control bg-background px-3 text-base outline-none sm:text-sm transition focus:border-ring focus:ring-4 focus:ring-ring/20" /><label htmlFor="password-next" className="text-sm font-semibold">New password</label><input id="password-next" aria-label="New password" type="password" autoComplete="new-password" value={password.next} onChange={(event) => setPassword({ ...password, next: event.target.value })} className="min-h-11 w-full rounded-lg border border-control bg-background px-3 text-base outline-none sm:text-sm transition focus:border-ring focus:ring-4 focus:ring-ring/20" /><div className="flex flex-wrap gap-2"><Button type="submit" disabled={Boolean(busy) || passwordState === "reconciling" || passwordState === "pending" || !password.current || password.next.length < 8}>{passwordRetry ? "Retry password change" : "Change password"}</Button><Button type="button" variant="secondary" onClick={() => void reconcilePassword()} disabled={Boolean(busy) || !passwordOperation}>Check password-change status</Button></div></form></section>
    <section className="space-y-3 rounded-lg border border-border bg-card p-4" aria-labelledby="offline-recovery-heading"><h2 id="offline-recovery-heading" className="font-editorial text-xl font-bold">Offline recovery codes</h2><p className="text-sm text-muted-foreground">Enroll, save, and rehearse a recovery code. Recovery always ends with a normal sign-in.</p><label htmlFor="recovery-current" className="text-sm font-semibold">Current password</label><input id="recovery-current" aria-label="Current password for recovery codes" type="password" autoComplete="current-password" value={recoveryPassword} onChange={(event) => setRecoveryPassword(event.target.value)} className="min-h-11 w-full rounded-lg border border-control bg-background px-3 text-base outline-none sm:text-sm transition focus:border-ring focus:ring-4 focus:ring-ring/20" /><div className="flex flex-wrap gap-2"><Button variant="secondary" onClick={() => void issueRecovery("enroll")} disabled={Boolean(busy) || recoveryState === "reconciling" || recoveryState === "pending" || !recoveryPassword}>{recoveryRetry && recovery?.action === "enroll" ? "Retry recovery enrollment" : "Create recovery codes"}</Button><Button variant="secondary" onClick={() => void issueRecovery("regenerate")} disabled={Boolean(busy) || recoveryState === "reconciling" || recoveryState === "pending" || !recoveryPassword}>{recoveryRetry && recovery?.action === "regenerate" ? "Retry recovery regeneration" : "Regenerate recovery codes"}</Button><Button variant="secondary" onClick={() => void reconcileRecovery()} disabled={Boolean(busy) || !recovery}>Check recovery status</Button></div>{recoveryCodes.length ? <div ref={displayOnceRecoveryCodesRef} role="region" aria-label="Display-once recovery codes" tabIndex={-1}><p className="font-semibold">Save these codes outside Cubby now.</p><ul className="grid grid-cols-2 gap-2 text-sm">{recoveryCodes.map((code) => <li key={code} className="rounded border border-border p-2 font-mono">{code}</li>)}</ul><Button onClick={() => void acknowledgeCodes()} disabled={Boolean(busy)}>I saved these codes</Button></div> : null}{recovery?.setVersion && !recoveryCodes.length ? <div className="space-y-2"><label className="text-sm font-semibold" htmlFor="rehearsal-code">Rehearse one unused code</label><input id="rehearsal-code" type="text" autoComplete="off" value={rehearsalCode} onChange={(event) => setRehearsalCode(event.target.value)} className="min-h-11 w-full rounded-lg border border-control bg-background px-3 text-base outline-none sm:text-sm transition focus:border-ring focus:ring-4 focus:ring-ring/20" /><Button variant="secondary" onClick={() => void rehearse()} disabled={Boolean(busy) || !rehearsalCode}>Rehearse code</Button></div> : null}</section>
    <section className="space-y-3 rounded-lg border border-border bg-card p-4" aria-labelledby="change-email-heading"><h2 id="change-email-heading" className="font-editorial text-xl font-bold">Change email address</h2><form aria-labelledby="change-email-heading" className="space-y-3" onSubmit={(event) => { event.preventDefault(); void initiateEmailChange(); }}><label htmlFor="email-current" className="text-sm font-semibold">Current password</label><input id="email-current" aria-label="Current password for email change" type="password" autoComplete="current-password" value={email.current} onChange={(event) => setEmail({ ...email, current: event.target.value })} className="min-h-11 w-full rounded-lg border border-control bg-background px-3 text-base outline-none sm:text-sm transition focus:border-ring focus:ring-4 focus:ring-ring/20" /><label htmlFor="email-next" className="text-sm font-semibold">New email address</label><input id="email-next" aria-label="New email address" type="email" autoComplete="email" value={email.next} onChange={(event) => setEmail({ ...email, next: event.target.value })} className="min-h-11 w-full rounded-lg border border-control bg-background px-3 text-base outline-none sm:text-sm transition focus:border-ring focus:ring-4 focus:ring-ring/20" /><Button type="submit" variant="secondary" disabled={Boolean(busy) || emailState === "reconciling" || emailState === "pending" || !email.current || !email.next}>{emailRetry ? "Retry email change" : "Send verification"}</Button></form>{emailPending ? <div className="space-y-2"><label htmlFor="email-verification" className="text-sm font-semibold">Verification material</label><input id="email-verification" type="text" autoComplete="off" value={email.verification} onChange={(event) => setEmail({ ...email, verification: event.target.value })} className="min-h-11 w-full rounded-lg border border-control bg-background px-3 text-base outline-none sm:text-sm transition focus:border-ring focus:ring-4 focus:ring-ring/20" /><div className="flex flex-wrap gap-2"><Button variant="secondary" onClick={() => void verifyEmailChange()} disabled={Boolean(busy) || emailStatus !== "pending" || !email.verification}>Verify new address</Button><Button onClick={() => void emailAction("cutover")} disabled={Boolean(busy) || emailStatus !== "verified"}>Complete email change</Button><Button variant="secondary" onClick={() => void emailAction("status")} disabled={Boolean(busy)}>Check email-change status</Button><Button variant="secondary" onClick={() => void emailAction("cancel")} disabled={Boolean(busy) || !["pending", "verified"].includes(String(emailStatus))}>Cancel email change</Button><Button variant="secondary" onClick={() => void emailAction("confirm")} disabled={Boolean(busy) || !(emailStatus === "issued" || emailCookieState === "issued")}>Confirm this device</Button></div></div> : null}{emailOldAddressNoticeFailed ? <p role="alert" className="rounded-lg border border-warning/40 bg-warning/10 p-3 text-sm text-foreground">The security notice to your old email address could not be delivered. Review your security history and active sessions.</p> : null}</section>
