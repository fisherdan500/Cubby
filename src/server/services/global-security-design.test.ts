...
describe("P1-3 global security design gate", () => {
  it("freezes the complete non-partial protocol state, route cutover, and privacy invariants", () => {
    ...
    expect(design.releaseGate).toMatchObject({
      forbidsPartialRelease: true,
      sequencingDecision: "DEC-PROD-408",
      activationBoundary: "existing_user_password_recovery_email_all_or_none_before_initial_credentials",
      initialCredentialSignup: "remain_denied_until_separate_invitation_onboarding_program"
    });
    ...
    expect(design.phase2AuthorizationBoundary).toEqual({ capture: "server_session_user_and_session_id_only", transaction: "serializable", reauthorization: "both_version_components_equal_or_stale_security_version", mutation: "callback_only_after_in_transaction_reauthorization", routeExposure: "selected_existing_user_activation_under_DEC_PROD_408" });
    expect(design.phase3FreshAuthBoundary).toEqual({ operationIdentity: "gso_crockford_26_database_enforced", issueOrder: ["reauthorize", "current_password_verify", "binding_open", "operation_pending", "grant_issued", "binding_submitted"], replay: "matching_submitted_unexpired_grant_only_without_password_reverify", status: "matching_binding_and_grant_only_with_expired_state_durably_terminalized", consumption: "matching_submitted_pending_operation_and_current_two_component_vector_only", routeExposure: "selected_existing_user_activation_under_DEC_PROD_408", passwordAttestation: { algorithm: "hmac_sha256", payloadVersion: "fresh-auth-attestation-v1", keyLifecycle: "active_plus_one_prior_for_ten_minute_overlap", nonce: "32_random_bytes_base64url_unique_write_once", replacementHashBinding: "sha256_of_exact_better_auth_password_hash", runtimeDatabaseThreat: "direct_cubby_runtime_sql_cannot_issue_password_change_grant_without_valid_attestation", protectedPurposes: ["password_change", "recovery_enrollment"] } });
    expect(design.phase4PasswordTransitionBoundary).toEqual({ hash: "before_serializable_transaction_with_better_auth_compatible_hasher", transactionWrites: ["grant_consumed", "credential_hash_replaced", "both_versions_advanced_attributed", "all_sessions_and_activity_revoked", "operation_completed_changed", "binding_terminal", "content_free_completed_event"], terminalOutcome: "completed|changed", postCommit: "normal_new_password_sign_in_required", routeExposure: "selected_existing_user_activation_under_DEC_PROD_408", credentialMutationReceipt: "security_definer_password_transition_procedure_with_attested_exact_replacement_hash_and_runtime_direct_write_denial", privilegeBoundary: "direct_cubby_runtime_sql_requires_owner_key_verified_one_time_password_attestation_and_cannot_substitute_replacement_hash", deploymentRoleLifecycle: "startup_provisions_runtime_role_then_migrates_then_owner_reconciles_versioned_attestation_keyring_before_unsetting_migration_secrets_and_starting_server" });
