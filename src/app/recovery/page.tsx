...
export default function RecoveryPage() {
  ...
  return <main className="mx-auto min-h-screen max-w-md space-y-5 px-3 py-12 md:px-8" aria-busy={busy ? "true" : "false"}><Link href="/login" className="text-sm font-bold text-primary">Back to sign in</Link><div><h1 className="font-editorial text-3xl font-bold">Recover your account</h1><p className="mt-2 text-sm text-muted-foreground">Use a saved offline recovery code. This process never signs you in automatically.</p></div><form aria-label="Recover your account" className="space-y-3 rounded-lg border border-border bg-card p-4" onSubmit={(event) => { event.preventDefault(); void submit(); }}><label htmlFor="recovery-email" className="text-sm font-semibold">Email address</label><input id="recovery-email" aria-label="Email address" type="email" autoComplete="email" disabled={busy} value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} className="min-h-11 w-full rounded-lg border border-control bg-background px-3 text-base outline-none sm:text-sm transition focus:border-ring focus:ring-4 focus:ring-ring/20" /><label htmlFor="recovery-code" className="text-sm font-semibold">Recovery code</label><input id="recovery-code" aria-label="Recovery code" type="text" autoComplete="off" disabled={busy} value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value })} className="min-h-11 w-full rounded-lg border border-control bg-background px-3 text-base outline-none sm:text-sm transition focus:border-ring focus:ring-4 focus:ring-ring/20" /><label htmlFor="recovery-new-password" className="text-sm font-semibold">New password</label><input id="recovery-new-password" aria-label="New password" type="password" autoComplete="new-password" disabled={busy} value={form.newPassword} onChange={(event) => setForm({ ...form, newPassword: event.target.value })} className="min-h-11 w-full rounded-lg border border-control bg-background px-3 text-base outline-none sm:text-sm transition focus:border-ring focus:ring-4 focus:ring-ring/20" /><Button type="submit" disabled={busy || !form.email || !form.code || form.newPassword.length < 8} className="w-full">Reset password</Button></form><p aria-live="polite" role="status" className="text-sm text-muted-foreground">{message}</p></main>;
}
